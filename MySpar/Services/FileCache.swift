...
final class FileCache {
    ...
    static func parseProducts() -> [ProductCard]? {
        if let fileURL = Bundle.main.url(forResource: "Directions", withExtension: "json") {
            if let jsonData = try? Data(contentsOf: fileURL) {
                do {
                    let json = try JSON(data: jsonData)
                    ...
                    for productJSON in productsArray {
                        ...
                        let price = productJSON[Keys.price.rawValue].stringValue
                        let oldPrice = productJSON[Keys.old_price.rawValue].int
                        let measurement = productJSON[Keys.measurement.rawValue].stringValue
                        let discount = productJSON[Keys.discount.rawValue].int
                        
                        let product = ProductCard(id: id, category: category, inf: inf, image: image, price: price, oldPrice: oldPrice, measurement: measurement, discount: discount)
